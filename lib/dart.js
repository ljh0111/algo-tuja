...
// 경우도 있어 최근 4개 연도까지, 사업보고서(11011)뿐 아니라 반기(11012)·3분기(11014) 보고서까지 시도합니다.
export async function fetchFinancialHighlights(corpCode) {
  ...
  for (const year of candidateYears) {
    for (const [reprtCode, reprtLabel] of reportCodes) {
      try {
        let list = await fetchOnce(corpCode, String(year), reprtCode, "CFS");
        if (!list || list.length === 0) {
          list = await fetchOnce(corpCode, String(year), reprtCode, "OFS");
        }
  ...
  // 시도한 조합이 모두 실패했고, 그중 진짜 에러(한도초과/인증키 등)가 있었다면 그 내용을 그대로 알려줍니다.
